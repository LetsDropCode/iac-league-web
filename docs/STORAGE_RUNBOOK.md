...
1. Dashboard inspection is complete and recorded in `STORAGE_VERIFICATION.md`. The live Free tier has no shell/SSH or disk, so **stop here for production**: upgrading would restart/redeploy and could erase ephemeral uploads. Ask Render Support for a non-restarting extraction path from the current instance, or obtain another independently verified current-live-data source. Only after safe access exists, use `ops/inspect_runtime.py` to capture UTC time, cwd, mount paths/filesystems, process roles/PIDs and environment **names only**. It never returns raw process arguments or environment values.
