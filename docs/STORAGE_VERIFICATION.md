...
| Seed inventory survives Render redeploy | PASS | 2026-09-22 / `dep-dapdaphsrm7s73f2uej0`, `dc81079` | Nine managed seed files remained; UI-imported results still require restart/redeploy test |
...
**Exact current gate:** the owner accepted the committed local recovery candidate as the authoritative production migration baseline on 21 September 2026, with the limitations recorded in `outputs/recovery/BASELINE-ACCEPTANCE.md`. AWS staging storage/monitoring and the isolated Render disk are active; secrets loaded, the mount and seed were observed, the seed survived redeploy, a real S3 upload/read-back verified, the fail-closed supervisor deployed, and its singleton/two-worker topology was proven. Still required: real UI synthetic import, restart and redeploy verification of that import, an actual scheduled backup, independent-machine restore, and failure/missing-monitor alert delivery. Do not upgrade, change environment/start commands, restart/redeploy, or attach a **production** disk until those isolated staging gates pass.
