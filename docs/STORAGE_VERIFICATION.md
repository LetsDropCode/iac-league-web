...
| Backup failure alarm and recovery | PASS; EMAIL RECEIPT PENDING | 2026-09-24 11:57–12:01 UTC / AWS staging stack | A staging-only temporary `s3:PutObject` deny caused `AccessDenied`, `BackupHealthy=0`, and ALARM at 11:57:57; permission was restored, a verified recovery backup ran, and alarm returned OK at 12:01:57. Temporary inline policy removed |
| Entire service/monitor missing alarm and recovery | PASS; EMAIL RECEIPT PENDING | 2026-09-24 12:40–12:52 UTC / `4ebac5a` | Confirmed metric cutoff 12:40:45; high-resolution 30-second, 3-of-3 alarm entered ALARM at 12:47:46 (about 7m01s, under the 10-minute gate). Permission restored at 12:49:06, `HEALTH_EMITTED=True`, alarm OK at 12:52:16. Temporary inline policy list verified empty |
...
**Exact current gate:** the owner accepted the committed local recovery candidate as the authoritative production migration baseline on 21 September 2026, with the limitations recorded in `outputs/recovery/BASELINE-ACCEPTANCE.md`. AWS staging storage/monitoring and the isolated Render disk are active. The mount, verified scheduled backup, independent empty-directory restore, restart/redeploy durability, correction/rollback, import freeze, two-worker visibility, pasted-provider formats, forced backup-failure alarm, and whole-monitor missing-signal alarm have passed. FinishTime's direct endpoint remains externally blocked specifically from Render, so the explicit external-search/paste fallback is the verified production path. Still required before production cutover: operator confirmation that the ALARM and OK/recovery emails arrived, the stale-backup-with-monitor-alive drill, and resource-capacity/timed-restore evidence. Do not upgrade, change environment/start commands, restart/redeploy, or attach a **production** disk until those remaining isolated staging gates pass.
