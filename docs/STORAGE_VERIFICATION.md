...
| Restored rules/mappings/checksums/standings match | PASS | 2026-09-24 / AWS CloudShell | 18 files; restored inventory SHA-256 `3098d0d3cbab1806bad2d4ee0b42cd07e07f110b147bb2f395bd62b8eb976a03`; all three labelled UI imports present |
...
**Exact current gate:** the owner accepted the committed local recovery candidate as the authoritative production migration baseline on 21 September 2026, with the limitations recorded in `outputs/recovery/BASELINE-ACCEPTANCE.md`. AWS staging storage/monitoring and the isolated Render disk are active. The mount, verified scheduled backup, independent empty-directory restore, restart/redeploy durability, correction/rollback, import freeze, two-worker visibility, pasted-provider formats, forced backup-failure alarm, whole-monitor missing-signal alarm, and external ALARM/OK email delivery have passed. FinishTime's direct endpoint remains externally blocked specifically from Render, so the explicit external-search/paste fallback is the verified production path. Still required before production cutover: the stale-backup-with-monitor-alive drill and resource-capacity/timed-restore evidence. Do not upgrade, change environment/start commands, restart/redeploy, or attach a **production** disk until those remaining isolated staging gates pass.
