...
Updated **2026-09-22**. Branch `codex/storage-durability`; the combined implementation and recovery-evidence commit `9ecedef` is present on `origin/codex/storage-durability`. Production remains at baseline commit `d285886d4347e11578a5667ff4870f7c035b95fe`; the storage implementation has **not** been deployed to production. Isolated Render staging first deployed commit `43f469090585afe678536df70556f3785e12cee4` successfully in maintenance-only mode. No production deployment, restart, disk attachment or live migration was performed.
...
The AWS stack `iac-league-staging-storage` reached `CREATE_COMPLETE` in `eu-north-1` at 19:13:08 UTC on 21 September 2026. Its reviewed change set contained exactly six additions: private encrypted/versioned S3 storage, TLS-only bucket policy, SNS topic/policy, CloudWatch missing/failure alarm, and restricted runtime IAM policy. All six resources reached `CREATE_COMPLETE`; expected bucket, alarm, and policy outputs exist. The SNS console subsequently showed the operator email subscription as `Confirmed`. The staging-only IAM user `iac-league-staging-runtime` was then created with console access disabled and exactly the restricted runtime policy attached directly. One access key was generated. Both AWS credential variable names were subsequently verified in the saved Render staging environment list without reading their values; runtime access has not yet been tested. Neither the access key identifier nor secret is recorded in repository evidence. Repository-safe evidence is in `outputs/staging/AWS-PROVISIONING.md`; physical identifiers and operator contact data are omitted.

Render staging service `srv-dap9at8473hc73dh0gn0` was provisioned on 22 September 2026 from `codex/storage-durability`. First deploy `dep-dap9atg473hc73dh0i1g` of `43f469090585afe678536df70556f3785e12cee4` succeeded in maintenance-only mode. The dashboard shows Starter-equivalent `0.5c-512mb`, a 1 GB disk mounted at `/var/data`, and Auto-Deploy Off. A read-only request to the staging URL returned HTTP 200 with exactly `League staging maintenance`. Neither the disk seed nor in-service backup supervisor/two Gunicorn workers has been run. The labelled synthetic fixture SHA-256 is `e8df6d47b2dc6b27c6ffef62188102324b7aae166b2f5a91bf7a21f5ef287c00`. Repository-safe detail is in `outputs/staging/RENDER-PROVISIONING.md`.

| Check | Status | Verified time / deployed commit | Missing evidence |
| --- | --- | --- | --- |
| Reviewed staging deployment inputs | PASS (AWS and Render staging provisioned) | 2026-09-22 / AWS template SHA-256 `a45a0f53e8fba7183e09fd73cee09d3a5bbf05f7861d60ba60f7d97ad5b2ac37`; Render commit `43f4690` | Provider-side secrets and disk seed |
| Staging tier and disk configuration | PASS (dashboard configuration only) | 2026-09-22 / Render deploy `dep-dap9atg473hc73dh0i1g` | Actual mount/device and disk ID; one supervisor/two workers not yet started |
...
**Exact current gate:** the owner accepted the committed local recovery candidate as the authoritative production migration baseline on 21 September 2026, with the limitations recorded in `outputs/recovery/BASELINE-ACCEPTANCE.md`. AWS staging storage and monitoring and an isolated Render staging service/disk are provisioned; the SNS subscription is confirmed. Both AWS credential variable names are saved in staging but have not been runtime-tested. A Render web-shell prompt appeared in Safari, but automated input did not reach it, so no disk seed, mount-device proof, scheduled backup, restart/redeploy durability, independent restore or alert-delivery test has passed. Do not upgrade, change environment/start commands, restart/redeploy, or attach a **production** disk until the isolated Render staging sequence and independent backup/restore evidence pass.
