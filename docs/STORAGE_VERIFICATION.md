...
Updated **2026-09-24**. Branch `codex/storage-durability`; the current staging application commit `4ebac5a` is present on `origin/codex/storage-durability` and was deployed successfully as `dep-daqhjjgjo6nc73efpdhg`. Production remains at baseline commit `d285886d4347e11578a5667ff4870f7c035b95fe`; the storage implementation has **not** been deployed to production. Isolated Render staging runs the fail-closed backup supervisor with the verified FinishTime provider-block fallback and high-resolution missing-signal monitoring. No production deployment, restart, disk attachment or live migration was performed.
...
| Stale backup while monitor remains alive | NOT RUN | — / unknown | Two-hour stale-age path and operator receipt remain |
...
**Exact current gate:** the owner accepted the committed local recovery candidate as the authoritative production migration baseline on 21 September 2026, with the limitations recorded in `outputs/recovery/BASELINE-ACCEPTANCE.md`. AWS staging storage/monitoring and the isolated Render disk are active. The mount, verified scheduled backup, independent empty-directory restore, restart/redeploy durability, correction/rollback, import freeze, two-worker visibility, pasted-provider formats, forced backup-failure alarm, whole-monitor missing-signal alarm, external ALARM/OK email delivery, resource-capacity capture, and timed restore have passed. FinishTime's direct endpoint remains externally blocked specifically from Render, so the explicit external-search/paste fallback is the verified production path. Still required before production cutover: the stale-backup-with-monitor-alive drill. Do not upgrade, change environment/start commands, restart/redeploy, or attach a **production** disk until that remaining isolated staging gate passes.
