...
**Exact current gate:** the owner accepted the committed local recovery candidate as the authoritative production migration baseline on 21 September 2026, with the limitations recorded in `outputs/recovery/BASELINE-ACCEPTANCE.md`. AWS staging storage/monitoring and the isolated Render disk are active; secrets loaded, the mount and seed were observed, a real S3 upload/read-back verified, the fail-closed singleton/two-worker supervisor deployed, and a real UI import survived both restart and full redeploy with exact checkpoint equality. Still required: paste/import behavior, an actual scheduled backup containing the UI import, independent-machine restore, correction/rollback/freeze exercises, and failure/missing-monitor alert delivery. Do not upgrade, change environment/start commands, restart/redeploy, or attach a **production** disk until those isolated staging gates pass.
